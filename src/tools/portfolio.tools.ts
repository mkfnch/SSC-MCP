...
export function registerPortfolioTools(server: McpServer, ssc: SecurityScorecardService): void {
  ...
  server.tool(
    ...
    {
      portfolio_id: opaqueIdSchema.describe('Portfolio ID'),
  ...
  server.tool(
    ...
    {
      name: z.string().min(1).max(200).describe('Portfolio name'),
      description: z.string().max(1000).optional().describe('Portfolio description'),
  ...
  server.tool(
    ...
    {
      portfolio_id: opaqueIdSchema.describe('Portfolio ID'),
      name: z.string().min(1).max(200).describe('New portfolio name'),
      description: z.string().max(1000).optional().describe('New portfolio description'),
    },
  ...
  server.tool(
    ...
    { portfolio_id: opaqueIdSchema.describe('Portfolio ID to delete') },
  ...
  server.tool(
    ...
    {
      portfolio_id: opaqueIdSchema.describe('Portfolio ID'),
      domain: domainSchema.describe('Company domain to add (e.g., example.com)'),
    },
  ...
  server.tool(
    ...
    {
      portfolio_id: opaqueIdSchema.describe('Portfolio ID'),
      domain: domainSchema.describe('Company domain to remove (e.g., example.com)'),
    },
    ({ portfolio_id, domain }) => handleVoidToolCall(
